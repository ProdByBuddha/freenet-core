...
pub async fn sync(config: SyncConfig, other: BaseConfig) -> anyhow::Result<()> {
    let sync_path = fs::canonicalize(&config.path)?;
    
    // Perform initial sync
    perform_sync(&sync_path, other.clone()).await?;

    if config.watch {
        println!("Watching for changes in {:?}...", sync_path);
        watch_directory(&sync_path, other).await?;
    }

    Ok(())
}

async fn perform_sync(sync_path: &Path, other: BaseConfig) -> anyhow::Result<()> {
...
async fn watch_directory(path: &Path, other: BaseConfig) -> anyhow::Result<()> {
    let (tx, mut rx) = mpsc::channel(100);
    
    let mut watcher = notify::recommended_watcher(move |res: Result<Event, notify::Error>| {
        if let Ok(event) = res {
            // Filter out events in the .freenet directory to avoid infinite loops
            let is_freenet_internal = event.paths.iter().any(|p| {
                p.components().any(|c| c.as_os_str() == ".freenet")
            });
            
            if !is_freenet_internal {
    ...
    // Debounce timer: wait for silence before syncing
        ...
        if let Some(_event) = rx.recv().await {
            ...
            loop {
                match tokio::time::timeout(debounce_duration, rx.recv()).await {
                    Ok(Some(_)) => {
                        // Got another event, restart timer
                        continue;
                    }
                    Ok(None) => break, // Channel closed
                    Err(_) => {
                        // Timeout reached, trigger sync
                        println!("Change detected, triggering sync...");
                        if let Err(e) = perform_sync(path, other.clone()).await {
...
mod tests {
    ...
    async fn test_sync_command_flow() -> anyhow::Result<()> {
        ...
        let sync_config = SyncConfig { path: path.to_path_buf(), watch: false };
